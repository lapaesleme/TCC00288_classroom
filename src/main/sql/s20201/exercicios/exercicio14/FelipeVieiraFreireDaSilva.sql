...
DECLARE
    time RECORD;
    participa RECORD;
    ligacoes_feitas integer:= 0;
    diferenca_tempo interval := 0;
    nBairro character varying;
    nMunicipio character varying;
    mediaLigacoes  float;
    tempoTotal float;
BEGIN
    FOR participa IN SELECT antena_orig AS antenaParticipante FROM ligacao WHERE inicio >= tempoIn AND fim <= tempoF UNION
                     SELECT antena_dest AS antenaParticipante FROM ligacao WHERE inicio >= tempoIn AND fim <= tempoF LOOP

        FOR time IN SELECT antena_orig, antena_dest, inicio, fim FROM ligacao WHERE inicio >= tempoIn AND fim <= tempoF LOOP
            IF time.antena_orig = participa.antenaParticipante OR time.antena_dest = participa.antenaParticipante THEN
                ligacoes_feitas = ligacoes_feitas + 1;
                diferenca_tempo = diferenca_tempo + time.Fim - time.inicio;
            END IF;
        END LOOP;

        tempoTotal := EXTRACT (minute fROM diferenca_tempo);
        tempoTotal := tempoTotal + EXTRACT(second FROM diferenca_tempo)/60;
        tempoTotal := tempoTotal + EXTRACT (hour FROM diferenca_tempo)*60;
        tempoTotal := tempoTotal + EXTRACT (day FROM diferenca_tempo)*24 *60;
        ...
        mediaLigacoes := tempoTotal/ligacoes_feitas;
